...
export class AppUtils {
    static uuid4(): string {
        return hyperid.decode(hyperid()()).uuid.replace(/-/g, '');
    }
    ...
    static maskPan(pan: string): string {
        return `${pan.substring(0, 4)} ${pan.substring(
            4,
            6,
        )}** **** ${pan.substring(12)}`;
    }
    ...
    static moment(date: Date) {
        return moment(date).tz('Asia/Ulaanbaatar');
    }

    static async sendEmail(subject: any, content: any, emails: any[]) {
        try {
            const tokenBody = qs.stringify({
                client_id: process.env.EMAIL_APPLICATION_ID,
            ...
            const tokenConfig = {
                ...
                url: `https://login.microsoftonline.com/${process.env.EMAIL_TENANT_ID}/oauth2/v2.0/token`,
                headers: {
            ...
            const emailConfig = {
                method: 'POST',
                maxBodyLength: Infinity,
                url: `https://graph.microsoft.com/v1.0/users/${process.env.EMAIL_SENDER}/sendMail`,
                headers: {
            ...
            await axios.request(emailConfig);
            return 'Амжилттай';
        } catch (error: any) {
            console.error(
                'Error:',
    ...
    static async sendSms(to: string[], text: string) {
        try {
            await Promise.all(
                to.map((recipient) =>
                    axios.get('https://api.messagepro.mn/send', {
                        params: {
            ...
            console.error(err?.response?.data?.reason || 'Алдаа гарлаа');
        }
    }
}
