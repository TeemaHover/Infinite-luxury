...
// Callback types
export type RowBatchHandler<T = any> = (rows: T[]) => Promise<void> | void;
...
export class AppDB {
        ...
        return this.executeQuery(async (client) => {
            try {
                ...
                if (result.rows.length === 0) {
                    // throw new AppDBResultNotFoundException(
                    //     'Select One not found!',
                    // );
                } else if (result.rows.length === 1) {
                    return result.rows[0];
                } else {
                    throw new AppDBTooManyResultException(
        ...
        return this.executeQuery(async (client) => {
            try {
                ...
                if (
                    ...
                    query = `${query} ORDER BY ${options.sortBy} ${
                        options.sortDir || 'ASC'
    ...
    /**
     * Execute a query using a server-side cursor and process rows in batches.
     * - No full result buffering; memory stays flat.
     * - Always closes the cursor and releases the client.
     *
     * @returns total rows processed
     */
    async selectBatchWithCursor<T = any>(
        sql: string,
        params: any[] = [],
        batchSize = 5000,
        onBatch?: RowBatchHandler<T>,
    ): Promise<number> {
        const client = await this.getConnection();
        let cursor: Cursor | null = null;
        let total = 0;

        const readBatch = (c: Cursor): Promise<T[]> =>
            new Promise((resolve, reject) => {
                c.read(batchSize, (err, rows: T[]) =>
        ...
        try {
            ...
            console.warn('Cursor select error!', `[${sql}], [${params}]`);
            throw err;
        } finally {
            try {
                if (cursor) {
                    await new Promise<void>((resolve, reject) =>
                        cursor!.close((e) => (e ? reject(e) : resolve())),
                    );
                }
            } finally {
                client.release();
